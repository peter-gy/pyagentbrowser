...
The central object is a `Browser` controller. It owns a native session, which sends ordered actions to a local browser process or an attached browser. A `Page` binds one browser target, while a `Frame` binds one child browsing context.
...
| `Page` | A main-document handle for the active tab or one exact target returned by `browser.tabs.get()`. |
| `Frame` | A child-document handle returned by `page.frames`. Its operations carry the same frame identity. |
...
`Browser.launch()` starts the browser before returning. `page.observe()` records the current accessibility tree. `Snapshot.one()` selects exactly one ref. `Ref.click()` performs the mutation, waits for the new URL, captures another snapshot with the same specification, and returns the transition evidence.
...
The optional `browser.cdp` API opens a direct [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/) connection for execution contexts, isolated worlds, and raw protocol methods.
