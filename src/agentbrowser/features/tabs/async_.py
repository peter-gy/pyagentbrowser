...
class AsyncTabs:
    ...
    async def list(self) -> tuple[TabInfo, ...]:
        """Return open tabs."""
        return await self.executor.execute(Command("tab_list", {}, decode=tabs_from_data))
        ...
        *,
        ...
        return await self.executor.execute(
            Command(
                "tab_switch",
                {
                    "tabId": await self._resolve_selector(
                        id=id, label=label, index=index, required=True
                    )
                },
